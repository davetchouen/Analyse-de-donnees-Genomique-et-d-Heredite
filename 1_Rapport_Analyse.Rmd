...
28 individus ont un coeficient d'apparentement supperieur ou egale a 0.25
...
options(gaston.autosomes = 1:26, gaston.chr.x = 27, gaston.chr.y = NA)
...
-Nous avons 7268 individus 
-On a 39176 snps
-Proportion de la maf inferieur à 0.05 est de 0.09
-Le callrate minimal des individus est de 0.09322034
-Le callrate minimal des snp est 0.5939736

```{r}
table(data_bed@snps$chr) # nombre de snps par chromosomes
```

Extraction du jeu de données où tous les SNPs ont un callrate supérieur ou égal à 95%, et une MAF supérieure
où égale à 5%
```{r}
...
En conclusion, le QQ-plot des p-valeurs pour le test des proportions de Hardy-Weinberg montre que les données sont approximativement en accord avec l'hypothèse de Hardy-Weinberg. Cependant, il y a quelques points qui s'écartent de la ligne verte, ce qui pourrait indiquer que l'hypothèse n'est pas vraie pour tous les loci 
...
correlation <- cor(10*coefficients_K, coefficients_grm)
```
##QUESTION 5
...
hist(beastx$IgAmp[(beastx$LambAdult == "Adults")],
     main = "Histogramme des valeurs d'IgA chez les Mouton", 
     xlab = "IgA")
```
...
#################################################
all(beastx$ID %in% data_bed@ped$id)##  
```
Tous les individus génotypés n'ont pas de données phenotypé

```{r}
beast_sans_na <- subset(beastx, !is.na(beastx$IgAmp))
Bed_disponible <- select.inds(data_bed, data_bed@ped$id %in% beast_sans_na$ID)
```
Extraction de la bed matrix des individus dont les données phenotypiques sont disponible 
...
dim(grm_moigneaux)
```
la dimention de la sous matrice grm des agneaux est de 2406*2406
...
l'heritabilité des agneaux est de 0.05732805
...
heritabilite_mouton 
```
l'heritabilité des Mouton est de 0.1285155
...
print(paste("Héritabilité chez les agneaux est avec la matrice k*2 :",heritabilite_agneaux_K) )
```
```{r}
nom_colonne_K <- colnames(K2)
...
aa <- association.test(bed_matrix_moigneaux,
                       Y=moigneaux_disponible$IgAmp, 
                       K = grm_moigneaux, method='lmm') ## Modele mixte 
```
...
Netoyage et arrangement des données 
...
aa_linear_chr24 <- association.test(bed_matrix_moigneauxchr24,
                                     Y=subset_chr24$ID, 
                                     K = grmchr24, method='lm', test = "wald")
```
